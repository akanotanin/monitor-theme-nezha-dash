...
function processServiceData(serviceData: ServiceData) {
	...
	// 只统计「当天真的探测过」的那几天：没有采样的日子 delay 是 0，
	// 混进平均数会把延迟拉成 0ms（极简探针的 ping 历史可能短于 30 天）。
	const measuredDelays = serviceData.delay.filter(
		(_, index) => serviceData.up[index] + serviceData.down[index] > 0,
	);
	const avgDelay =
