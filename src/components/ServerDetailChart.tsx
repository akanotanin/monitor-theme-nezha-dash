...
function MemChart({
	...
	// Fetch memory history; no swap history is available
	...
	useEffect(() => {
		...
		const fetchMemData = async () => {
			...
			try {
				const memResponse = await fetchServerMetrics(
					data.id,
					"memory",
					period as MetricPeriod,
				);
				...
				if (memResponse.success && memResponse.data?.data_points) {
					const memoryData = memResponse.data.data_points.map((point) => {
					...
					if (!cancelled) {
						setMemHistoricalData(memoryData);
					}
				}
			} catch (error) {
	...
	}, [data.id, period, data.host.mem_total]);
		...
		if (
			...
			const historyData = messageHistory
				.map((wsData) => {
					const server = wsData.servers.find((s) => s.id === data.id);
					if (!server) return null;
					const { mem } = formatNezhaInfo(wsData.now, server);
					return {
						timeStamp: wsData.now.toString(),
						mem,
					};
	...
	useEffect(() => {
		if (data && historyLoaded && period === "realtime") {
			...
			setMemChartData((prevData) => {
				...
				if (prevData.length === 0) {
					newData = [
						{ timeStamp: timestamp, mem },
						{ timeStamp: timestamp, mem },
					];
				} else {
					newData = [...prevData, { timeStamp: timestamp, mem }];
					if (newData.length > 30) {
						newData.shift();
					}
				}
