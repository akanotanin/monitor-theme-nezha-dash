...
**带宽**（蓝标签）与 **IPv4／IPv6**（紫／粉标签）探针没有数据源（公开接口 `/api/nodes` 不暴露 IP，后台只有 `ipv4_pin`／`ipv6_pin` 两个管理字段），所以不自动生成。
卡片底部标签行统一居中：上游按父级 `align-items` 对齐，视口宽于 1024px（`lg` 断点）时是左对齐、窄屏才居中（回退删掉 `PlanInfo.tsx` 的 `w-full`／`justify-center` 即可）。
